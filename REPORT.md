...
The handoff package defines session-bound requests, a stable session/intervention-scoped operator credential, actor ownership, epoch fencing, reconciliation outcomes, and safe state callbacks. Actor and protocol tests cover stale claims, duplicate claims, draining, and wrong-principal reconciliation. The full application handoff wiring and a browser-backed operator intervention are not complete in this release state.
...
The following limits are deliberate and are recorded as `NOT_RUN` or pending rather than presented as completed product behavior: every V1–V12 acceptance status in the current verifier output, including live model discovery (V1), ten native cross-customer replays with a balance change (V2), real-person handoff (V9), clean-checkout reproduction (V11), and browser end-to-end operation-page completion (V12). The user deferred provider credentials, so no live-qualified capability digest or real approved replay artifact is included.
