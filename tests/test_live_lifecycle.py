...
from scripts import discover_and_replay, review_native_bundle
...
def test_live_runner_requires_explicit_opt_in(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="CUA_LIVE"):
        discover_and_replay._require_explicit_opt_in()


def test_live_runner_paths_are_pinned_to_the_executable_contract_version():
    assert discover_and_replay._artifact_paths("1.0.0") == (
        "artifacts/get_savings_balance-1.0.0.json",
        "evidence/live_lifecycle.json",
        "evidence/live_run",
    )
    with pytest.raises(ValueError, match="1.0.0"):
        discover_and_replay._artifact_paths("1.0.1")
